...
N_ATTEMPT_MOVE = 2  # number of attempts to move a walls
...
class Interface(Plugin):
    ...
    def qt_callback_plotSaveBtn_clicked(self):
        ...
        file_name, _ = QFileDialog.getSaveFileName(
            None, "Save CSV File", dir_path, "CSV Files (*.csv);;All Files (*)", options=options)
        ...
        if file_name:
            ...
            self._widget.fileDirEdit.setText(dir_path)
            self.updateFileListWidget(dir_path, os.path.basename(file_name))
    ...
    def updateFileListWidget(self, dir_path, selected_file_name=None):
        """ Update the fileListWidget with the CSV files in the specified directory.
        ...
        self._widget.fileListWidget.addItems(csv_files)

        # If a selected file name is provided, set the currentRow to that file
        if selected_file_name:
            items = self._widget.fileListWidget.findItems(selected_file_name, Qt.MatchExactly)
            if items:
                index = self._widget.fileListWidget.row(items[0])
                self._widget.fileListWidget.setCurrentRow(index)
