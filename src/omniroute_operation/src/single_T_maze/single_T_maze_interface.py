    ...
    def run_experiment(self):
        ...
        if self.mode == Mode.START_EXPERIMENT:
            ...
            if (self.current_time - self.mode_start_time) >= self.right_choice_delay:
                if self.phase == ExperimentPhases.PHASE_THREE and self.success_chamber == self.left_chamber and self.currentTrialNumber >= self.switch_trial + 18:
                    self.switch_trial = self.currentTrialNumber
            ...
            if (self.current_time - self.mode_start_time) >= self.wrong_choice_second_delay:
                if self.phase == ExperimentPhases.PHASE_THREE and self.error_chamber == self.left_chamber and self.currentTrialNumber >= self.switch_trial + 18:
                    self.switch_trial = self.currentTrialNumber
