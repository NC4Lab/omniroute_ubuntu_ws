...
private:
	...
	struct GRBLSetting
	{
		const char *command; // The GRBL command string, e.g., "$0"
	...
	GRBLSetting _grblSettings[70] = {
		...
		{"$110", 30000.000},  // X-axis maximum rate, mm/min [0.0 - 100000.0]
		{"$111", 30000.000},  // Y-axis maximum rate, mm/min [0.0 - 100000.0]
...
public:
	uint8_t grblWrite(const String &cmd, bool do_wait_ack = true, unsigned long timeout = 1000);
