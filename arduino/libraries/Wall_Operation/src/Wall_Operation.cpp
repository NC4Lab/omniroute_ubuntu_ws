...
uint8_t Wall_Operation::testWallIO(uint8_t cham_i, uint8_t p_wall_inc[], uint8_t s)
{
	...
	uint8_t r_bit_out;
	uint8_t resp = 0;
	_Dbg.printMsg("START", "RUNNING: Test IO switches: chamber=%d wall=%s", cham_i, _Dbg.arrayStr(p_wi, s));
	while (true)
	{ // loop indefinitely
		for (size_t i = 0; i < s; i++)
		{ // loop walls
	...
	_Dbg.printMsg("ERROR", "Test IO switches: chamber=%d wall=%s", cham_i, _Dbg.arrayStr(p_wi, s));
	return resp;
}
...
uint8_t Wall_Operation::testWallPWM(uint8_t cham_i, uint8_t p_wall_inc[], uint8_t s, uint32_t dt_run)
{
	...
	_Dbg.printMsg("START", "RUNNING: Test PWM: chamber=%d wall=%s", cham_i, _Dbg.arrayStr(p_wi, s));
	uint8_t resp = 0;
	for (size_t i = 0; i < s; i++)
	{ // loop walls
		uint8_t wall_n = p_wi[i];
...
uint8_t Wall_Operation::testWallOperation(uint8_t cham_i, uint8_t p_wall_inc[], uint8_t s)
{
	...
	// Test all walls
	_Dbg.printMsg("START", "RUNNING: Test move opperation: chamber=%d wall=%s", cham_i, _Dbg.arrayStr(p_wi, s));
	uint8_t r_bit_out = 1;
	uint16_t dt = 2000;
	uint16_t ts;
	uint8_t resp = 0;
