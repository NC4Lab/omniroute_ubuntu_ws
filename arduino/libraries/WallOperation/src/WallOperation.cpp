...
void WallOperation::initSoftware(uint8_t init_level, uint8_t setup_arg_arr[])
{
	...
	if (init_level == 0 || init_level == 1)
	{
		...
		for (size_t cham_i = 0; cham_i < nCham; cham_i++)
		{
			...
			_Dbg.printMsg(_Dbg.MT::INFO, "CHAMBER[%d] ADDR[%s] WALLS[%s]", cham_i, _Dbg.hexStr(C[cham_i].addr), _Dbg.binStr(C[cham_i].bitWallExist));
		}
	...
	_Dbg.printMsg(_Dbg.MT::ATTN, "FINISHED: SOFTWARE %s", init_level == 0 ? "INITIALIZATION" : init_level == 1 ? "REINITIALIZATION"
																												 : "RESET");
}
...
{
	_Dbg.printMsg(_Dbg.MT::ATTN, "START: CYPRESS INITIALIZATION");
	...
	// Print status
	_Dbg.printMsg(i2c_status == 0 ? _Dbg.MT::ATTN : _Dbg.MT::ERROR,
...
uint8_t WallOperation::initWalls(uint8_t init_level)
{
	...
	_Dbg.printMsg(_Dbg.MT::ATTN, "START: WALL %s INITIALIZATION",
				  init_level == 0 ? "UP/DOWN" : "DWON");

	// Save wall errors
	uint8_t bit_wall_error_flag[nChamInitMax] = {0};
	...
	if (init_level == 0)
	{
		...
		// Copy over error flag and reset to ensure down move runs
		for (size_t cham_i = 0; cham_i < nCham; cham_i++)
		{
			bit_wall_error_flag[cham_i] = C[cham_i].bitWallErrorFlag;
			C[cham_i].bitWallErrorFlag = 0;
		}
	}
	...
	if (init_level == 0 || init_level == 1)
	{
		// Set wall move for all chambers
		/// @note: Set to include err flagged walls for init/reinit
		for (size_t cham_i = 0; cham_i < nCham; cham_i++)
		...
		// Explicitly check wall state and set position
		for (size_t cham_i = 0; cham_i < nCham; cham_i++)
			getWallState(cham_i, 1, C[cham_i].bitWallPosition);
	}

	// Include wall erros from either run
	for (size_t cham_i = 0; cham_i < nCham; cham_i++)
		C[cham_i].bitWallErrorFlag = C[cham_i].bitWallErrorFlag | bit_wall_error_flag[cham_i];

	//............... Check Status ...............

	// Print status
	_Dbg.printMsg(run_status <= 1 ? _Dbg.MT::ATTN : _Dbg.MT::ERROR,
...
/// @endcode
uint8_t WallOperation::setWallsToMove(uint8_t cham_i, uint8_t pos_state_set)
{
...
/// @endcode
uint8_t WallOperation::setWallsToMove(uint8_t cham_i, uint8_t pos_state_set, uint8_t p_wall_inc[], uint8_t s)
{
	// Handle array inputs
	if (s > 8)
		return -1;
...
/// @endcode
uint8_t WallOperation::setWallsToMove(uint8_t cham_i, uint8_t pos_state_set, uint8_t byte_wall_state_new)
{
	// Run private version of the method
	return _setWallsToMove(cham_i, pos_state_set, byte_wall_state_new);
}
...
uint8_t WallOperation::_setWallsToMove(uint8_t cham_i, uint8_t pos_state_set, uint8_t byte_wall_state_new)
{
...
uint8_t WallOperation::moveWallsByChamberBlocks()
{
	...
	_Dbg.printMsg(_Dbg.MT::ATTN, "START: STAGED MOVE WALL: chambers%s", _Dbg.arrayStr(cham_all_arr, n_cham_all));
	...
	if (run_status <= 1)
		_Dbg.printMsg(_Dbg.MT::ATTN, "FINISHED: STAGED MOVE WALL: status[%d] blocks[%d] attempts[%d] chambers%s",
...
uint8_t WallOperation::_monitorWallsMove(uint8_t cham_i)
{
	...
	for (size_t prt_i = 0; prt_i < C[cham_i].pmsActvIO.nPortsInc; prt_i++) // loop ports
	{
		...
		uint8_t io_change_check_byte = C[cham_i].pmsActvIO.byteMaskInc[prt_i] & io_in_reg[port_n];
