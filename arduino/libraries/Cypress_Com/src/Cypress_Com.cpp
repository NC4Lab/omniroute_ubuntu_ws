...
/// <param name="p_byte_val_out">Byte array from the register (used as output).</param>
/// <param name="s">Length of p_byte_val_out [1-16].</param>
/// <returns>Wire::method output [0-4] or [-1=255:input argument error].</returns>
uint8_t Cypress_Com::i2cRead(uint8_t address, uint8_t reg, uint8_t p_byte_val_out[], uint8_t s) {
	if (s > 16) return -1;
...
/// <summary>
/// Overload option to pass an array of bytes "array "p_byte_val_in" to set multiple 
/// registers begining at register specified by "reg". 
/// </summary>
/// <param name="p_byte_val_in">Byte pointer array with the registry values.</param>
/// <param name="s">Length of the "p_byte_val_in" array [1-16]</param>
uint8_t Cypress_Com::i2cWrite(uint8_t address, uint8_t reg, uint8_t p_byte_val_in[], uint8_t s) {
	if (s > 16) return -1;
	Wire.beginTransmission(address);
	Wire.write(reg);
	for (size_t i = 0; i < s; i++) {
		Wire.write(p_byte_val_in[i]);
	}
	return wireEndTransmissionWrapper();
}
...
void Cypress_Com::_updateRegByte(uint8_t& r_byte_val_out, uint8_t byte_mask, uint8_t bit_val_set) {
	if (bit_val_set == 1) { //set bit in byte1 to 1 if it or the corresponding bit in byte2 is equal to 1
		r_byte_val_out = r_byte_val_out | byte_mask;
	}
	else { //set bit in byte1 to 0 if the corresponding bit is equal to 1 in byte2
		r_byte_val_out = r_byte_val_out & ~byte_mask;
	}
}
...
uint8_t Cypress_Com::ioReadPin(uint8_t address, uint8_t port, uint8_t pin, uint8_t& r_bit_val_out) {
	if (port > 5 || pin > 7) return -1;
	else {
		uint8_t port_byte;
		uint8_t resp = i2cRead(address, REG_GI0 + port, &port_byte, 1);
		delay(1); //hack to deal with strange resp variable behavior
		if (!resp) {
			r_bit_val_out = bitRead(port_byte, pin);
		}
		else return resp;
	}
}
...
uint8_t Cypress_Com::ioWritePin(uint8_t address, uint8_t port, uint8_t pin, uint8_t bit_val_set) {
	if (port > 5 || pin > 7) return -1;
		...
		uint8_t port_byte;
		uint8_t resp = i2cRead(address, REG_GO0 + port, &port_byte, 1); //get current port registry value
		if (!resp) {
			bitWrite(port_byte, pin, bit_val_set); //set pin specific bit of uint8_t
			uint8_t resp2 = i2cWrite(address, REG_GO0 + port, port_byte); //update register
			return resp2;
...
/// <param name="address">I2C address for a given Cypress chip.</param>
/// <param name="reg">Register to read from.</param>
/// <param name="p_byte_val_out">Byte pointer array with the registry values (used as output).</param>
/// <param name="s">Length of the "p_byte_val_out" array [1-16]</param>
/// <returns>Wire::method output [0-4] or [-1=255:input argument error].</returns>
uint8_t Cypress_Com::ioReadReg(uint8_t address, uint8_t reg, uint8_t p_byte_val_out[], uint8_t s) {
	if (s > 16) return -1;
	uint8_t resp = i2cRead(address, reg, p_byte_val_out, s);
	return resp;
}
...
/// </summary>
/// <param name="address">I2C address for a given Cypress chip.</param>
/// <param name="p_byte_mask">Byte value pointer array in which bits set to one denote the pin/bit to set in the registers.</param>
/// <param name="s">Length of the "p_byte_mask" array [1-16]</param>
/// <param name="bit_val_set">Value to set the bits to [0,1].</param> 
/// /// <param name="p_byte_mask">OPTIONAL: pointer byte array of the previous registry. Should be same s as "s".</param>
/// <returns>Wire::method output [0-4] or [-1=255:input argument error].</returns>
uint8_t Cypress_Com::ioWriteReg(uint8_t address, uint8_t p_byte_mask[], uint8_t s, uint8_t bit_val_set, uint8_t p_reg_byte_last[]) {
	if (s > 16) return -1;
	uint8_t p_byte_val[s]; //initialize array to handle null array argument
	if (p_reg_byte_last == nullptr) {
		uint8_t resp = ioReadReg(address, REG_GO0, p_byte_val, s); //get current registry values
		if (resp) return resp;
	}
	else { // copy over input
		for (size_t i = 0; i < s; i++) p_byte_val[i] = p_reg_byte_last[i];
	}

	for (size_t i = 0; i < s; i++) {
		_updateRegByte(p_byte_val[i], p_byte_mask[i], bit_val_set);
	}
...
uint8_t Cypress_Com::i2cScan() {
	...
	for (addr = 1; addr < 127; addr++)
	{
		...
		if (err == 0) { // check for repsonse
			list_addr[cnt_addr] = addr;
			cnt_addr++;
		}
		else if (err == 4) { //catch unknown error
	...
	if (cnt_addr > 0) {
		...
		for (size_t i = 0; i < cnt_addr; i++) { //print devices
	...
	if (cnt_err > 0) {
		...
		for (size_t i = 0; i < cnt_addr; i++) { //print errors
